...
def test_user_without_id(client, get_csv_data):
    csv_users = get_csv_data
    test_user = csv_users[0]
    res = client.get(f"{url_for('display_user')}")
    client_user = res.get_data().decode('utf-8')
    try:
        assert client_user == test_user
    except AssertionError as err:
        print(err)
        pytest.fail(f"조회한 유저 아이디 1 의 유저 {test_user} 가 받은 응답의 유저 {client_user} 와 일치하지 않습니다.")
